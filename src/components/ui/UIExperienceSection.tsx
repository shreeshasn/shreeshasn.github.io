...
export const UIExperienceSection: React.FC = () => {
  const { workExperience, projects, setSelectedProjectId, isSoundEnabled } = usePortfolio();
    ...
    <section id="experience" className="section-spacing content-container" style={{ borderBottom: '1px solid var(--color-hairline)' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-lg)' }}>
          {visible.map((exp) => {
            const project = projects.find((p) => p.id === exp.relatedProjectId);
            const projectTitle = project ? project.title : exp.relatedProjectId;
                ...
                {exp.relatedProjectId && (
                  ...
                  >
                    <span style={{ fontSize: '0.8125rem', color: 'var(--color-body)', fontFamily: 'var(--font-mono)' }}>
                      [!] Linked Project: <strong style={{ color: 'var(--color-ink)' }}>{projectTitle}</strong>
                    </span>
                    <button
                      onClick={() => {
